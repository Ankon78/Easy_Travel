...
function search($key){
	$con = getConnection();
	$query = "SELECT AccountantNo from accountant where AccountantNo like '%$key%'";
...
if(isset($_POST['type']) == 1){
	$AccountantNo =$_POST['AccountantNo'];
	 $query ="SELECT * FROM accountant where AccountantNo ='".$AccountantNo."' ";
	$result =mysqli_query($con, $query);
